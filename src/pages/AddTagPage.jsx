...
export default function AddingPage() {
  ...
  const { data1, loading1, error1, reFetch } = useFetch("https://apiblognode.onrender.com/api/tags/", token);
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`https://apiblognode.onrender.com/api/tags/${currentId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  
      // Refresh the data after deleting the tag
      reFetch();
  ...
  const handleAddCategory = async (inputValue) => {
    try {
      const res = await axios.post(
        "https://apiblognode.onrender.com/api/tags/",
  ...
  const handleEditopen = async () => {
    ...
    const handleEdit = async (name) => {
      try {
        ...
        const response = await axios.put(`https://apiblognode.onrender.com/api/tags/${currentId}`, updatedCategory, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
...
const cat = "Tag"
  const handleDeleteall = async () => {
    try {
      ...
      const res = await axios.delete("https://apiblognode.onrender.com/api/tags", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        data: { tagsIds: selectedIds }, // Pass the array of selected IDs in the request body
      });

      // Refresh the data after deleting the tags
      reFetch();
      setSelected([])
    } catch (error) {
  ...
  return (
    <>
      {isModalOpen && <CatModal name={"Tag"} icon={<LocalOfferIcon />} open={true} handleAddCategory={handleAddCategory} handleClose={handleCloseModal} />}
      <Helmet>
