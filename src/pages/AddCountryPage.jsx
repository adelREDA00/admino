...
export default function AddingPage() {
  ...
  const { data1, loading1, error1, reFetch } = useFetch("/api/country/", token);
...
const trackId = (id,name)=>{
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`/api/country/${currentId}`, {
        headers: {
...
const handleEditopen = async () => {
  ...
  const handleEdit = async (name) => {
    try {
      ...
      const response = await axios.put(`/api/country/${currentId}`, updatedCategory, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
